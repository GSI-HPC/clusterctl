...
type Streams struct {
	...
	// Display says a progress display, the live tree, the counter or plain
	// lines, is shown on standard error while the command runs, through Out
	// and Err, which keep what the command writes clear of it. Nothing else
	// may draw there then: scp's progress meter is left off.
	Display bool
