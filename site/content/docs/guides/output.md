...
On a terminal, a command that has run for a second shows the work under way as
a live tree at the bottom of standard error, and leaves a summary line behind.
In a pipe, a file or a CI log nothing is shown unless `--progress plain` asks
for lines: standard error holds exactly what it holds with `--progress none`,
and progress never reaches standard output, whatever `-o` says.
`--progress-log` keeps every event in a file of JSON lines for a program to
read. [Progress](../progress/) describes all of it.
